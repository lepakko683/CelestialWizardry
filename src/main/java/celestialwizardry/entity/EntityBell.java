			...
			if(plrs != null && plrs.size()>0) {
				...
				for(int i=0;i<plrs.size();i++) {
					...
					if(targett.getDisplayName().equalsIgnoreCase(playerToFollow)) {
						...
						try{
							double dist = Math.sqrt(Math.pow(Math.abs(this.posX-targett.posX),2) + Math.pow(Math.abs(this.posY-targett.posY),2) + Math.pow(Math.abs(this.posZ-targett.posZ),2));
