...
public class OBJModelBlock extends WavefrontObject {
	...
	public void tessellateAllWAS(Tessellator tes, double x1, double y1, double x2, double y2, double x, double y, double z) {
	...
	private void renderGroupObjectWAS(Tessellator tes, GroupObject go, double x1, double y1, double x2, double y2, double x, double y, double z) {
		if(go.faces.size() > 0) {
			for(Face f : go.faces) {
	...
	private void addFaceForRenderWAS(Tessellator tes, double x1, double y1, double x2, double y2, Face f, float texOffs, double x, double y, double z) {
...
//                System.out.println("U: " + RenderHelper.scaleUVCoord(f.textureCoordinates[i].u + offsetU, x1, x2) + " V: " + RenderHelper.scaleUVCoord(f.textureCoordinates[i].v + offsetV, y1, y2));
                tes.addVertexWithUV(x+f.vertices[i].x, y+f.vertices[i].y, z+f.vertices[i].z, RenderHelper.scaleUVCoord(f.textureCoordinates[i].u + offsetU, x1, x2), RenderHelper.scaleUVCoord(f.textureCoordinates[i].v + offsetV, y1, y2));
            }
            else
            {
