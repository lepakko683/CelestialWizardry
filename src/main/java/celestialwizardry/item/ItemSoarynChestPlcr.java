...
import celestialwizardry.util.MiscHelpers;
import celestialwizardry.util.Triple;
...
public class ItemSoarynChestPlcr extends ItemSingle {
	...
	public boolean onItemUse(ItemStack item, EntityPlayer player, World world, int x, int y, int z, int side, float par8, float par9, float par10) {
		if(!world.isRemote) {
			...
			if(Blocks.chest.canPlaceBlockAt(world, placeCrds.getA(), placeCrds.getB(), placeCrds.getC()) && world.isAirBlock(placeCrds.getA(), placeCrds.getB(), placeCrds.getC())) {
				...
				Blocks.chest.onBlockPlacedBy(world, placeCrds.getA(), placeCrds.getB(), placeCrds.getC(), player, placer);
				...
				if(te instanceof TileEntityChest) {
					for(int i=0;i < (9*3);i++) {
						if(player.inventory.getStackInSlot(i+9) != null) {
							((TileEntityChest)te).setInventorySlotContents(i, player.inventory.getStackInSlot(i+9).copy());
