...
public class Loc3DF {
	...
	public float getDistanceTo(Location3D b) {
		return (float) Math.sqrt( Math.pow( this.x - b.getX(), 2 ) + Math.pow( this.y - b.getY(), 2 ) + Math.pow( this.z - b.getZ(), 2 ) );
	}
	...
	public static float getDistance(float x, float y, float z, Vec3 vec) {
		return (float) Math.sqrt( Math.pow( x - vec.xCoord, 2 ) + Math.pow( y - vec.yCoord, 2 ) + Math.pow( z - vec.zCoord, 2) );
	}
}
