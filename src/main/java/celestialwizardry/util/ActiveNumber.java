    ...
    public double update(double value)
    {
        ...
        switch (this.mode)
        {
            case MODE_OSCILLATE:
                if (this.increase)
                {
                    this.actValue = MathHelper
                            .clampDouble(this.actValue + Math.abs(value), this.bounds_min, this.bounds_max);
                }
                else
                {
                    this.actValue = MathHelper
            ...
            case MODE_OSCILLATE_SMOOTH:
                double delta = MathHelper.clampDouble(
                        value * (Math.toDegrees(Math.atan(distanceToBounds(this.actValue) / (distBetweenBounds() / 2)))
                                / 45d), this.min_osc_value, value);
                if (this.increase)
                {
                    this.actValue = MathHelper
                            .clampDouble(this.actValue + delta, this.bounds_min - 0.05d, this.bounds_max + 0.05d);
                }
                else
                {
                    this.actValue = MathHelper
...
//			System.out.println("Angs: " + (Math.toDegrees(Math.atan(distanceToBounds(this.actValue)/(distBetweenBounds
// ()/2))))/45d);
