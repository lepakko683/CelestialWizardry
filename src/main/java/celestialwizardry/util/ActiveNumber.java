...
public class ActiveNumber {
	...
	public double update(double value) {
		...
		case MODE_OSCILLATE:
			...
			if(this.actValue <= this.bounds_min+0.0001d) { // Small marginal because of the nature of non-whole numbers
				this.increase = true;
			}
			if(this.actValue >= this.bounds_max-0.0001d) { // Small marginal because of the nature of non-whole numbers
				this.increase = false;
			}
			break;
...
//			System.out.println("Angs: " + (Math.toDegrees(Math.atan(distanceToBounds(this.actValue)/(distBetweenBounds()/2))))/45d);
			if(this.actValue <= this.bounds_min+0.05d) { // Small marginal because of the nature of non-whole numbers
				this.increase = true;
			}
			if(this.actValue >= this.bounds_max-0.05d) { // Small marginal because of the nature of non-whole numbers
				this.increase = false;
			}
			break;
