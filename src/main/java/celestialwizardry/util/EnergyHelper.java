...
import celestialwizardry.api.energy.IEnergyContainer;
...
public class EnergyHelper
{
    ...
    public static float getTransformRatio(EnergyType source, EnergyType target)
    {
    	if(source.getEnergyCategory() != null && target.getEnergyCategory() != null) {
    		if (source.getEnergyCategory() instanceof EnergyType.CWEnergyCategory && source.getEnergyCategory().equals(EnergyType.CWEnergyCategory.MAGIC))
            {
    ...
    {
    	return (isEnergyOfCategory(source, EnergyType.CWEnergyCategory.MAGIC) && isEnergyOfCategory(target, EnergyType.CWEnergyCategory.MAGIC)) || 
    			(isEnergyOfCategory(source, EnergyType.CWEnergyCategory.MAGIC) && isEnergyOfCategory(target, EnergyType.CWEnergyCategory.ELEMENTAL)) ||
    			(isEnergyOfCategory(source, EnergyType.CWEnergyCategory.ELEMENTAL) && isEnergyOfCategory(target, EnergyType.CWEnergyCategory.MAGIC)) ||
    			source.canBeTransformedInto(target);
//        return (source.isMagicalEnergy() && target.isMagicalEnergy()) || (source.isMagicalEnergy() && target
//                .isElementalEnergy()) || (source.isElementalEnergy() && target.isMagicalEnergy()) || source.canBeTransformedInto(target);
    }
