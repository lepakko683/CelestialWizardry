...
public class ItemExpireEventHandler
{
    ...
    public void onEvent(ItemExpireEvent event)
    {
        if (!event.entityItem.worldObj.isRemote)
        {
            ...
            {
                event.entityItem.worldObj.spawnEntityInWorld(
                        new EntityItem(event.entityItem.worldObj, event.entityItem.posX, event.entityItem.posY,
                                       event.entityItem.posZ, new ItemStack(ModItems.material, 1, 3)));
            }
        }
    }
}
