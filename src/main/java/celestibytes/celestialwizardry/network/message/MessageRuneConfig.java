...
import celestibytes.celestialwizardry.handler.ClientRuneConfigurationHandler;
...
public class MessageRuneConfig implements IMessage, IMessageHandler<MessageRuneConfig, IMessage>
{
    ...
    public void fromBytes(ByteBuf buf)
    {
        ...
        while ((cc = (char) buf.readByte()) != endOfEntries && cycles < entryCount && cycles < entryHardLimit
                && cysLeft > 1)
        {
            if (cc == entryEndChar)
            {
    ...
    public void toBytes(ByteBuf buf)
    {
        ...
        for (int i = 0; i < Math.min(entryLines.length, entryCount); i++)
        { // Prevents array index out of bounds exception, but will cause further problems with invalid amount of runeconfig entries
            sb = new StringBuilder();
